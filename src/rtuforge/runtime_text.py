...
TEXT: dict[str, dict[str, str]] = {
    "en": {
        ...
        "help_run": """run script <name> [-d|--decode|-r|--raw]\nrun file <file> [--script <name>] [-d|--decode|-r|--raw]\n\nRun a stored script, or run a portable .rtus file without importing it. A bundle requires --script. Relative paths use the process CWD.\n\nExamples:\n  run script idd-status -r\n  run file demo.rtus --decode\n  run file backup.rtus --script idd-status -r\n\nDecode priority:\n  flag on an individual send > flag on run script/run file > runtime.decode_rx.\n\nFor one-shot mode, -c/--clean is a global CLI flag and may be placed before or after the command. It is not a run-file flag in the interactive shell.\n""",
        "help_export": """export script <name> --file <path> [--overwrite]\nexport scripts --file <path> [--overwrite]\n\nExport one or all stored scripts as a UTF-8 scripts-v1 .rtus file. Relative paths use the process CWD. Existing files are preserved unless --overwrite is supplied.\n""",
        "help_import": """import script <file> [--overwrite]\nimport scripts <file> [--overwrite]\n\nImport one script or merge a bundle into scripts.ini. Conflicts are rejected atomically unless --overwrite is supplied; unrelated stored scripts are preserved. Relative paths use the process CWD.\n""",
    ...
    "ru": {
        ...
        "help_run": """run script <name> [-d|--decode|-r|--raw]\nrun file <файл> [--script <name>] [-d|--decode|-r|--raw]\n\nВыполнить сохранённый скрипт или переносимый .rtus без импорта. Для bundle требуется --script. Относительные пути считаются от CWD процесса.\n\nПримеры:\n  run script idd-status -r\n  run file demo.rtus --decode\n  run file backup.rtus --script idd-status -r\n\nПриоритет: флаг отдельного send > флаг run script/run file > runtime.decode_rx. В one-shot -c/--clean является глобальным CLI-флагом, а не флагом run file интерактивной консоли.\n""",
        "help_export": """export script <name> --file <путь> [--overwrite]\nexport scripts --file <путь> [--overwrite]\n\nЭкспорт одного или всех скриптов в UTF-8 файл .rtus формата scripts-v1. Относительные пути считаются от CWD. Существующий файл сохраняется без --overwrite.\n""",
        "help_import": """import script <файл> [--overwrite]\nimport scripts <файл> [--overwrite]\n\nИмпорт одного скрипта или атомарное слияние bundle со scripts.ini. Конфликты отклоняются без --overwrite, остальные сохранённые скрипты не удаляются. Относительные пути считаются от CWD.\n""",
